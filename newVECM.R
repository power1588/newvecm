...
#close price
Afdataclo <- data.frame(Ahs300, Azz500, Asz50, Aif00, Aic00,Aih00)
#return
...
Afdatavol <- data.frame(hs300vol[yf1:yf3],zz500vol[1:yc3],sz50vol[1:yc3],if00vol[yf1:yf3],
                        ic00vol[1:yc3],
                        ih00vol[1:yc3])


names(Afdatavol) <- c("hs300_vol","zz500vol","sz50_vol","if00_vol",
                      "ic00_vol",
                      "ih00_vol")

Afdatapos <- data.frame(if00pos[yf1:yf3],
                        ic00pos[1:yc3],
                        ih00pos[1:yc3])

names(Afdatapos) <- c("if00_pos", 
                      "ic00_pos",
                      "ih00_pos")

#416 to 1231 mean of volume and position
mean_Aret <- sapply(1:dim(AfdataDcl)[2], function(i) mean(AfdataDcl[,i]))

#stand error of return
std_Aret <- sapply(1:dim(AfdataDcl)[2], function(i) sd(AfdataDcl[,i]))
#annulize
sd_Aret <- std_Aret*sqrt(240*252)

#acf
acf_Aret <- sapply(1:dim(AfdataDcl)[2], function(i) acf(AfdataDcl[,i], lag.max = 1)$acf)
...
#mean of return
mean_Bret <- sapply(1:dim(BfdataDcl)[2], function(i) mean(BfdataDcl[,i]))
mean_Bret <- as.data.frame(mean_Bret)

#stand error of return
std_Bret <- sapply(1:dim(BfdataDcl)[2], function(i) sd(BfdataDcl[,i]))
#annulize
sd_Bret <- std_Bret*sqrt(240*252)

#acf
acf_Bret <- sapply(1:dim(BfdataDcl)[2], function(i) acf(BfdataDcl[,i], lag.max = 1)$acf)
...
#mean of return
mean_Cret <- sapply(1:dim(CfdataDcl)[2], function(i) mean(CfdataDcl[,i]))

#stand error of return
std_Cret <- sapply(1:dim(CfdataDcl)[2], function(i) sd(CfdataDcl[,i]))
#annulize
#some question
sd_Cret <- std_Cret*sqrt(240*252)

#acf
acf_Cret <- sapply(1:dim(CfdataDcl)[2], function(i) acf(CfdataDcl[,i], lag.max = 1)$acf)
...
# for VECM the lag level we use 5.
lagv <- 5
...
GHdata <- function(HS300, ZZ500, SZ50, IF00,
                   IC00, IH00,  tid = Atid, N = 1, K="G"){
...
#GG measure for 3 spot-futures pairs in Period A
Aggdata <- GHdata(Ahs300, Azz500, Asz50, Aif00,
                  Aic00, 
                  Aih00, tid = Atid
)

#test for IH
#co1 <- cotestA(Asz50, Aih00, n=1)
#summary(co1)
#co2 <- cotestA(Asz50, Aih00, n=2)
#summary(co2)



#Hasbrouck for 3 spot-futures pairs in Period A
Ahadata <- GHdata(Ahs300, Azz500, Asz50, Aif00, 
                  Aic00, 
                  Aih00, tid = Atid, K="H"
)

#FOR 5-MIN
#GG measure for 3 spot-futures pairs in Period A with 5-min interval
Agg5data <- GHdata(Ahs300, Azz500, Asz50, Aif00, 
                   Aic00, 
                   Aih00, tid = Atid, N = 5
)
#Hasbrouck for 3 spot-futures pairs in Period A with 5-min interval
Aha5data <- GHdata(Ahs300, Azz500, Asz50, Aif00, 
                   Aic00,  
                   Aih00,  tid = Atid, K="H", N = 5
)

# 10 min
Agg10data <- GHdata(Ahs300, Azz500, Asz50, Aif00,
                    Aic00,
                    Aih00,  tid = Atid, N = 10
)
#Hasbrouck for 3 spot-futures pairs in Period A with 5-min interval
Aha10data <- GHdata(Ahs300, Azz500, Asz50, Aif00,
                    Aic00, 
                    Aih00,  tid = Atid, K="H", N = 10
)

# 30 min
Agg30data <- GHdata(Ahs300, Azz500, Asz50, Aif00,
                    Aic00,
                    Aih00, tid = Atid, N = 30
)
#Hasbrouck for 3 spot-futures pairs in Period A with 5-min interval
Aha30data <- GHdata(Ahs300, Azz500, Asz50, Aif00,  
                    Aic00, 
                    Aih00,  tid = Atid, K="H", N = 30
)
...
Bggdata <- GHdata(Bhs300, Bzz500, Bsz50, Bif00,
                  Bic00,
                  Bih00,  tid = Btid)

#Hasbrouck measure
Bhadata <- GHdata(Bhs300, Bzz500, Bsz50, Bif00, 
                  Bic00, 
                  Bih00,  tid = Btid, K = "H")
# 5 min
#GG measure for 12 spot-futures pairs in Period A
Bgg5data <- GHdata(Bhs300, Bzz500, Bsz50, Bif00,
                   Bic00,
                   Bih00, tid = Btid, N = 5)

#Hasbrouck measure
Bha5data <- GHdata(Bhs300, Bzz500, Bsz50, Bif00,
                   Bic00, 
                   Bih00,  tid = Btid,N =5, K = "H")

# 10 min
#GG measure for 12 spot-futures pairs in Period A
Bgg10data <- GHdata(Bhs300, Bzz500, Bsz50, Bif00,
                    Bic00, 
                    Bih00,  tid = Btid, tid = Btid, N = 10)

#Hasbrouck measure
Bha10data <- GHdata(Bhs300, Bzz500, Bsz50, Bif00,
                    Bic00, 
                    Bih00,  tid = Btid, tid = Btid,N = 10, K = "H")

# 30 MIN
#GG measure for 12 spot-futures pairs in Period A
Bgg30data <- GHdata(Bhs300, Bzz500, Bsz50, Bif00,
                    Bic00, 
                    Bih00,  tid = Btid, tid = Btid, N = 30)

#Hasbrouck measure
Bha30data <- GHdata(Bhs300, Bzz500, Bsz50, Bif00,
                    Bic00, 
                    Bih00,  tid = Btid, tid = Btid,N = 30, K = "H")
...
Cggdata <- GHdata(Chs300, Czz500, Csz50, Cif00, 
                  Cic00,  
                  Cih00,  tid = Ctid)

#Hasbrouck measure
Chadata <- GHdata(Chs300, Czz500, Csz50, Cif00, 
                  Cic00, 
                  Cih00,  tid = Ctid, K = "H")

# 5 min
#GG measure for futures
Cgg5data <- GHdata(Chs300, Czz500, Csz50, Cif00, 
                   Cic00, 
                   Cih00, tid = Ctid, N = 5)

#Hasbrouck measure
Cha5data <- GHdata(Chs300, Czz500, Csz50, Cif00, 
                   Cic00, 
                   Cih00, tid = Ctid,N = 5, K = "H")

# 10 min
#GG measure for futures
Cgg10data <- GHdata(Chs300, Czz500, Csz50, Cif00, 
                    Cic00, 
                    Cih00, tid = Ctid, N = 10)

#Hasbrouck measure
Cha10data <- GHdata(Chs300, Czz500, Csz50, Cif00, 
                    Cic00, 
                    Cih00, tid = Ctid,N = 10, K = "H")

# 30 min
#GG 
Cgg30data <- GHdata(Chs300, Czz500, Csz50, Cif00, 
                    Cic00, 
                    Cih00, tid = Ctid, N = 30)

#Hasbrouck measure
Cha30data <- GHdata(Chs300, Czz500, Csz50, Cif00, 
                    Cic00, 
                    Cih00, tid = Ctid,N = 30, K = "H")
